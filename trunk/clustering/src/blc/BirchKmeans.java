...
public class BirchKmeans extends ClusteringModel implements Serializable {
  ...
  public int buildGlobalDictionaryFromSerializedRawDocuments (
    ...
    for (int i = 0; i < files.length; ++i) {
      ...
      if (files[i].isDirectory()) {
        numDocumentsAdded +=
          this.buildGlobalDictionaryFromSerializedRawDocuments(
            ...
            outputDirectory + File.separator + files[i].getName());
      ...
      if (files[i].isFile() &&
        ...
        try {
          Document doc =
            Document.deserializeDocument(files[i].getAbsolutePath());
  ...
  private int clusterDocument (Document doc) {
    ...
    switch (this.clusterOptions.getClusteringApproach()) {
      case BEST_FIT_ALLOCATION:
        for (int i = 0; i < this.clusters.size(); ++i) {
          ...
          double proposedChangeInQuality =
            c.calculateChangeInQuality(this.getNormalizedDocumentVector(doc));
          ...
          if ((proposedChangeInQuality < qualityDeltaForBestCluster) &&
              (newQuality <
               upperQualityBound * (c.getNumberOfDocuments() + 1))) {
            bestClusterIdx = i;
      ...
      case REASONABLE_EFFORT_FORWARD:
        int clustersToCheck =
          (int) Math.ceil(this.clusterOptions.getReasonableEffortValue() *
                          this.clusters.size());

        for (int i = 0; (i < this.clusters.size()) && (clustersToCheck > 0);
            ++i) {
          BirchCluster c = this.clusters.get(i);
      ...
      case REASONABLE_EFFORT_BACKWARD:
        int clustersToCheckB =
          (int) Math.ceil(this.clusterOptions.getReasonableEffortValue() *
                          this.clusters.size());

        for (int i = this.clusters.size() - 1;
            (i >= 0) && (clustersToCheckB > 0); --i) {
          BirchCluster c = this.clusters.get(i);
    ...
    if (bestClusterIdx >= 0) {
      this.clusters.get(bestClusterIdx).addDocument(this, doc,
                        qualityForBestCluster);
  ...
  public int clusterDocumentsInMemory (boolean startOver) {
    ...
    Hashtable<String, Document> docsInMemory = new Hashtable<String,
                                                             Document>();
    ...
    while (pqCopy.peek() != null) {
      ...
      if (this.beVerbose()) {
        System.out.println(pqCopy.size() +
                           " documents remaining to be loaded " +
                           "into memory");
      }
   ...
   * @return
   */
  public String getCSV (long startTime,
                        long endTime) {
    StringBuffer sb = new StringBuffer();
  ...
  public double getGlobalQuality () {
    ...
    globalQuality = globalQuality -
                    (s.lengthSquared() * this.getNumberOfDocuments());
  ...
  public double getGlobalSparsity () {
    return (1.0 * this.numNonZeroEntries) /
           (1.0 * this.getNumberOfDistinctTerms() *
            this.getNumberOfDocuments());
  }
   ...
   * @return
   */
  public String getSQL (long startTime,
                        long endTime) {
    StringBuffer sb = new StringBuffer();
    ...
    if (this.blockNewDocumentsAndTermReduction == true) {
      ...
      if (this.clusterOptions.getTermReductionApproach() ==
        ...
        if ((bc.getTermReductionApproach() !=
             TermReductionApproach.USE_TERM_REDUCTION) || (this.clusterOptions
               .getMaxTermLimit() != bc.getMaxTermLimit())) {
          throw new Exception(
            "After normalized documents have been created, " +
